...
try {
    $url = Get-ManifestUrl
    if (!$url) {
        Show-Message "Es ist noch keine Update-Adresse eingerichtet. FÃƒÂ¼r Updates musst du eine Manifest-URL in ccusage-config.json setzen." "Claude Usage Tray Update" "Warning"
        exit 2
    }
    ...
    $notes = ""
    if ($manifest.notes) { $notes = "`n`nÃƒâ€žnderungen:`n$($manifest.notes)" }
    ...
    if (!(Test-Path -LiteralPath (Join-Path $source "ccusage-tray.ps1"))) {
        throw "Update-ZIP enthÃƒÂ¤lt keine ccusage-tray.ps1."
    }
