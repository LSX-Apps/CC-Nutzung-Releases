...
function Stop-Tray {
    try {
        $procs = Get-CimInstance Win32_Process | Where-Object {
            ...
            $_.CommandLine -and
            $_.CommandLine -match "\\-File\\s+[^`r`n]*ccusage-tray\\.ps1"
        }
...
try {
    $url = Get-ManifestUrl
    if (!$url) {
        Show-Message "Es ist noch keine Update-Adresse eingerichtet. Fuer Updates musst du eine Manifest-URL in ccusage-config.json setzen." "Claude Usage Tray Update" "Warning"
        exit 2
    }
    ...
    $notes = ""
    if ($manifest.notes) { $notes = "`n`nAenderungen:`n$($manifest.notes)" }
    ...
    if (!(Test-Path -LiteralPath (Join-Path $source "ccusage-tray.ps1"))) {
        throw "Update-ZIP enthaelt keine ccusage-tray.ps1."
    }
