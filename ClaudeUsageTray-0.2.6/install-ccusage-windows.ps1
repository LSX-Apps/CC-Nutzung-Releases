...
function Show-CredentialHint {
    ...
    Write-Host "Einmalig noetig:"
    Write-Host "  1. Claude Code installieren"
    Write-Host "  2. In PowerShell einmal 'claude' starten und den Browser-Login abschliessen"
    Write-Host "  3. Danach dieses Install-Skript erneut ausfuehren oder im Tray 'Jetzt aktualisieren' klicken"
    Write-Host ""
}
...
$runVbs = Join-Path $InstallDir "ccusage-run.vbs"

if (!$NoAutostart) {
    New-StartupShortcut -TargetVbs $runVbs
    Write-Host "Autostart angelegt: $ShortcutPath" -ForegroundColor Green
} else {
    Write-Host "Autostart wurde uebersprungen."
}

New-DesktopShortcut -TargetVbs $runVbs
Write-Host "Desktop-Verknuepfung angelegt: $DesktopShortcutPath" -ForegroundColor Green
