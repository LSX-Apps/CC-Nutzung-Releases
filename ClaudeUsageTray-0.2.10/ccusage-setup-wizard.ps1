...
$Intro.Text = "Dieser Assistent installiert Claude Code, startet den Login und richtet danach das Tray-Icon fÃ¼r die Claude-Nutzung ein."
...
$RefreshButton.Text = "Erneut prÃ¼fen"
...
$ReadmeButton.Text = "Anleitung Ã¶ffnen"
...
function Refresh-State {
    ...
    if ($hasCreds) {
        ...
        if (!$hasClaude) {
            $LoginStatus.Text = "Gesperrt. Bitte fÃƒÆ’Ã‚Â¼hre zuerst Schritt 1 aus."
            $LoginStatus.ForeColor = $ColorTextDisabled
        } else {
            $LoginStatus.Text = "Ausstehend. Bitte logge dich ÃƒÆ’Ã‚Â¼ber den Button ein."
    ...
    if ($isInstalled) {
        ...
        if (!$hasCreds) {
            $InstallStatus.Text = "Gesperrt. Bitte fÃƒÆ’Ã‚Â¼hre zuerst Schritt 2 aus."
...
try {
  ...
  Write-Host "Du kannst dieses Fenster jetzt schlieÃƒÆ’Ã…Â¸en und im Setup 'Erneut prÃƒÆ’Ã‚Â¼fen' klicken." -ForegroundColor Gray
} catch {
  Write-Host ""
...
'@
    ...
    Show-Info "Es wurde ein Terminal geÃƒÆ’Ã‚Â¶ffnet. Warte, bis die Installation dort fertig ist, schlieÃƒÆ’Ã…Â¸e das Terminal und klicke hier auf 'Erneut prÃƒÆ’Ã‚Â¼fen'."
})

$LoginButton.Add_Click({
    $claude = Find-Claude
    if (!$claude) {
        Show-Warn "Claude Code wurde noch nicht gefunden. Bitte zuerst Schritt 1 ausfÃƒÆ’Ã‚Â¼hren."
        return
    }
...
& "$claude"
Write-Host ""
Write-Host "Wenn der Login fertig ist, dieses Fenster schlieÃƒÆ’Ã…Â¸en und im Setup 'Erneut prÃƒÆ’Ã‚Â¼fen' klicken." -ForegroundColor Gray
"@
    Start-VisiblePowerShellScript -ScriptText $script
})
...
$CloseButton.Add_Click({
    ...
    if ($isInstalled) {
        ...
        $result = [System.Windows.Forms.MessageBox]::Show(
            "Die Installation ist noch nicht vollstÃƒÆ’Ã‚Â¤ndig eingerichtet. MÃƒÆ’Ã‚Â¶chtest du das Setup wirklich abbrechen?", 
            "Setup abbrechen", 
            "YesNo", 
            "Question"
        )
