...
function New-UsageIcon {
    ...
    $circlePen = New-Object System.Drawing.Pen $Color, 6
    $symbolPen = New-Object System.Drawing.Pen $Color, 8
    $symbolPen.StartCap = [System.Drawing.Drawing2D.LineCap]::Round
    $symbolPen.EndCap = [System.Drawing.Drawing2D.LineCap]::Round
    $symbolBrush = New-Object System.Drawing.SolidBrush $Color

    # Draw outer outline circle, leaving the inside transparent
    $graphics.DrawEllipse($circlePen, 7, 7, 50, 50)
    ...
    if ($null -eq $pct) {
        ...
        $graphics.DrawLine($symbolPen, 32, 17, 32, 36)
        $graphics.FillEllipse($symbolBrush, 27.5, 42.5, 9, 9)
    } else {
        $graphics.DrawLine($symbolPen, 18, 33, 28, 43)
        $graphics.DrawLine($symbolPen, 28, 43, 46, 22)
    }
