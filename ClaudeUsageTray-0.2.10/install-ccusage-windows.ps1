...
function Show-CredentialHint {
    ...
    Write-Host "Einmalig nÃ¶tig:"
    Write-Host "  1. Claude Code installieren"
    Write-Host "  2. In PowerShell einmal 'claude' starten und den Browser-Login abschlieÃŸen"
    Write-Host "  3. Danach dieses Install-Skript erneut ausfÃ¼hren oder im Tray 'Jetzt aktualisieren' klicken"
    Write-Host ""
}
...
if (!$NoAutostart) {
    ...
    Write-Host "Autostart wurde Ã¼bersprungen."
}

New-DesktopShortcut -TrayScript $trayScript
Write-Host "Desktop-VerknÃ¼pfung angelegt: $DesktopShortcutPath" -ForegroundColor Green
