...
function Refresh-TokenViaClaudeCli {
    try {
        $cmd = Get-Command claude -ErrorAction SilentlyContinue
        if (!$cmd) {
            return $false
...
try {
    ...
    try {
        ...
        if ($VerboseOutput) {
            Write-Host "Standard-Token-Refresh fehlgeschlagen. Starte CLI-Fallback..."
        }
        if (Refresh-TokenViaClaudeCli) {
            if (Test-Path -LiteralPath $credentialsPath) {
                $credentials = Get-Content -LiteralPath $credentialsPath -Raw | ConvertFrom-Json
                $oauth = $credentials.claudeAiOauth
                if ($oauth) {
                    ...
                    try { $expiresAt = [Int64]$oauth.expiresAt } catch {}
                    $nowMs = [DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds()
                    if ($expiresAt -and $expiresAt -gt $nowMs) {
                        $token = $oauth.accessToken
                        if ($VerboseOutput) {
                            Write-Host "CLI-Fallback erfolgreich! Neues Token geladen."
                        }
                    }
                }
            }
